...
type UserStorage struct {
	path      string
	usersList map[string]*user.User
}

func newUserStorage(path string) *UserStorage {
	u := UserStorage{}
	u.path = path
	u.usersList = make(map[string]*user.User)
	return &u
}
...
func (us *UserStorage) GetUser(name string) (*user.User, error) {
...
func (us *UserStorage) readUser(name string) (*user.User, error) {
...
func (us *UserStorage) SaveUser(u *user.User) error {
	if u == nil || u.Name == "" {
